...
int main(int argc, char ** argv) {
  ...
  //   const std::string filename = "story2";
  ...
  //iterate the graph
  queue<vector<pair<size_t, size_t> > > myqueue;
  page current_page;
  size_t cur_page = 0;
  myqueue.push(vector<pair<size_t, size_t> >(1, make_pair(0, 0)));

  while (!myqueue.empty()) {
    //xxx
    //destination num, choice
    vector<pair<size_t, size_t> > way = myqueue.front();
    cur_page = way.back().first;
    current_page = mystory.getpage(cur_page);
    myqueue.pop();

    if (current_page.getmychar() == 'W') {
      //print it
      for (vector<pair<size_t, size_t> >::iterator it = way.begin(); it != way.end() - 1;
           ++it) {
        cout << it->first << "(" << it->second << "),";
      }
      cout << cur_page << "(win)" << endl;
      continue;
    }

    for (size_t i = 0; i < current_page.sizeofmychoices(); i++) {
      size_t next_page = current_page.getmydes(i);
      bool exist = false;
      for (vector<pair<size_t, size_t> >::iterator it = way.begin(); it != way.end();
           ++it) {
        if (it->first == next_page) {
          exist = true;
          break;
        }
      }
      if (!exist) {
        vector<pair<size_t, size_t> > newWay = way;
        newWay.back().second = i + 1;
        newWay.push_back(make_pair(next_page, 0));
        myqueue.push(newWay);
      }
    }
  }
